...
void renderSort(SDL_Renderer* renderer, const std::vector<int>& arr, size_t currentIndex, size_t secondIndex, const std::string& mode) {
    ...
    for (size_t i = 0; i < arr.size(); ++i) {
        ...
        if (mode == "update") {
            SDL_SetRenderDrawColor(renderer, 0x00, 0xFF, 0x00, 0xFF); // Green for new array
...
void handleEvents() {
    ...
    while (SDL_PollEvent(&e) != 0) {
        if (e.type == SDL_QUIT) {
            ...
            else if (e.key.keysym.sym == SDLK_0) { // Create new array and visualize it
                std::vector<int> newArr(80);
                for (int i = 0; i < 80; ++i) {
                    newArr[i] = std::rand() % 100; // Random values between 0 and 99
                }
                for (int i = 0; i < 6; ++i) {
                    renderSort(renderers[i], newArr, 0, 0, "update");
                }
                clearScreen();
                std::cout << "New array generated and visualized.\n";
            }
        }
    }
}
...
void insertionSort(std::vector<int>& arr, SDL_Renderer* renderer) {
    for (size_t i = 1; i < arr.size(); ++i) {
        ...
        // Correct the loop to avoid out-of-bounds access
...
void executeSorting(int option) {
    ...
    // Do not set quit flag here, allow the windows to persist
}
    ...
    while (running) {
        ...
        switch (menuChoice) {
            case 1: {
                ...
                sortingThread.join(); // Wait for sorting to finish
                close();
                quit = false;
                break;
            }
